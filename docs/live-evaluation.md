...
Crie `.runtime/live-authorization.json` somente após autorizar a execução, com os campos:

```json
{
  "authorized": false,
  "approval_id": "identificador-da-autorizacao",
...
# Com os casos finais escritos, congela código/casos sem abri-los:
...
O comando `--freeze` recusa sobrescrever um freeze anterior. Se desenvolvimento exigir mudança antes da avaliação final, preserve o freeze anterior com a identificação da execução e registre o novo contrato; não renomeie resultados. A etapa final de 24×2 tem critério fixo; resultado reprovado continua reprovado até correção e nova avaliação autorizada.
...
<a id="referências-conferidas-em-21092026"></a>
...
[runner](../evals/live_runner.py) · [orçamento da avaliação](../evals/live_budget.py) · [critérios semânticos](../evals/semantic.py).
