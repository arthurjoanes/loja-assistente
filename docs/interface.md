...
A composição em revisão está detalhada na [matriz de qualidade](frontend-quality.md); suas jornadas ainda não foram executadas. Sem resposta, **Nova consulta** mostra controles e pergunta antes dos atalhos. Com resposta, a leitura ocupa a área principal, e **Próxima consulta** fica abaixo. Até 1024 px, esta preparação pode ser recolhida e reaberta por **Editar pergunta**. A barra do produto reúne Análises, Atendimentos e Nova análise; **Histórico** abre as conversas anteriores. Dados de acesso e métricas ficam em um bloco recolhível.
